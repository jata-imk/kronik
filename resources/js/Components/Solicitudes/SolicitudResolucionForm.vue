...
<template>
    <form v-if="opciones.length" class="space-y-3 rounded border border-surface-200 p-4 dark:border-surface-700" @submit.prevent="guardar">
        ...
        <template v-if="form.accion">
            ...
            <div v-if="form.accion === 'devolver'" class="flex flex-col gap-1"><label for="sol-corrector">Responsable de corregir</label><Select input-id="sol-corrector" aria-label="Responsable de corregir" v-model="form.responsable_id" :options="responsables" option-label="name" option-value="id" filter filter-placeholder="Buscar por nombre" empty-filter-message="No hay responsables con ese nombre." /></div>
