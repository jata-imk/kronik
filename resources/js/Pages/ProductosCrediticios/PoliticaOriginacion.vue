...
const form = useForm({ version_anterior: props.politica?.numero ?? 0, modalidad: anteriores.modalidad ?? null, sic: anteriores.sic ?? null, monto_maximo: anteriores.monto_maximo == null ? null : Number(anteriores.monto_maximo), vigencia_dias: anteriores.vigencia_dias ?? "", documentos: anteriores.documentos ?? [], referencia_validacion: "", criterio_capacidad: anteriores.criterio_capacidad ?? "", confirmacion: false });
function guardar() {
    form.transform(data => ({ ...data, monto_maximo: data.monto_maximo == null ? null : Number(data.monto_maximo).toFixed(2) })).post(route("originacion-politicas.store", props.version.id), { preserveScroll: true, onSuccess: () => { form.version_anterior = props.politica.numero; form.confirmacion = false; form.referencia_validacion = ""; } });
}
</script>
<template>
        ...
        <template #card-content>
            <div class="originacion-content space-y-5 p-4 md:p-6">
                ...
                <form class="space-y-4" @submit.prevent="guardar">
                    ...
                    <div class="flex flex-col gap-1"><label for="pol-maximo">Límite de aprobación (MXN)</label><InputNumber input-id="pol-maximo" v-model="form.monto_maximo" mode="currency" currency="MXN" locale="es-MX" :min-fraction-digits="2" :max-fraction-digits="2" fluid required /><small>Dentro del rango del producto: {{ money(version.monto_minimo) }} a {{ money(version.monto_maximo) }} MXN.</small></div>
