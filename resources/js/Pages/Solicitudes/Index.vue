...
const props = defineProps({ solicitudes: Object, filters: Object, miTrabajo: Boolean, puedeCrear: Boolean });
...
const estados = [{ label: "Todos", value: null }, { label: "Borrador", value: "borrador" }, { label: "En revisión", value: "en_revision" }];
function buscar(page = 1) {
    router.get(route(props.miTrabajo ? "solicitudes.trabajo" : "solicitudes.index"), { ...filters, page }, {
...
<template>
    <AppLayout :title="miTrabajo ? 'Mi trabajo' : 'Solicitudes'">
        <template #card-header>
            <div class="flex flex-wrap items-center justify-between gap-3 p-6">
                <h1 class="text-2xl font-semibold">{{ miTrabajo ? "Mi trabajo" : "Solicitudes" }}</h1>
        ...
        <template #card-content>
            <div class="space-y-5 p-6">
                <p>{{ miTrabajo ? "Solicitudes bajo tu responsabilidad. Abre una para ver el siguiente paso." : "Seguimiento de captura y envío a revisión. Este incremento todavía no permite aprobar ni desembolsar." }}</p>
                ...
                <DataTable :value="solicitudes.data" :loading="loading" striped-rows scrollable data-key="id">
                    ...
                    <Column header="Estado"><template #body="{ data }">{{ data.estado === "borrador" ? "Borrador" : "En revisión" }}</template></Column>
                    <Column field="responsable.name" header="Responsable" />
                    <Column field="sucursal.nombre" header="Sucursal" />
                    <Column header="Siguiente paso"><template #body="{ data }">{{ data.estado === "borrador" ? "Completar captura y enviar" : "Evaluación y cumplimiento pendientes" }}</template></Column>
                </DataTable>
